...
- **Утренний дайджест** (`MORNING_DIGEST_TIME`, по умолчанию 08:00) — сколько
  ...
  встречи из Google Calendar на сегодня (если настроен) и главная задача дня.
  Погода/пробки — пока не реализовано.
...
tests/                              — 93 теста на все модули выше
...
- Утренний дайджест теперь включает встречи из Google Calendar (если
  настроен) — погода/пробки пока не подключены.
...
- Идеи на будущее: погода/пробки в дайджест (Open-Meteo, без ключа),
  embeddings/second brain, режим webhook для облачного деплоя.
