...
- **Утренний дайджест** (`MORNING_DIGEST_TIME`, по умолчанию 08:00) — сколько
  открытых задач, какие просрочены (тег `@ГГГГ-ММ-ДД` в тексте задачи) и
  главная задача дня. Погода/пробки/встречи из календаря — в планах, пока не
  реализовано.
...
bot/
  ...
  services/
    ...
    digest_service.py              — утренний дайджест, вечерняя рефлексия, /review
  ...
  scheduler/
    jobs.py                        — APScheduler: дайджест/рефлексия/обзор по расписанию
tests/                              — 71 тест на все модули выше
...
- Утренний дайджест пока строится только по задачам из Obsidian — интеграция
  с погодой/пробками/Google Calendar в планах.
...
- Идеи на будущее: Google Calendar (read-only → создание встреч), подготовка
  к встрече (context brief), embeddings/second brain, режим webhook для
  облачного деплоя.
