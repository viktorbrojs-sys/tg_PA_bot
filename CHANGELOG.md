...
## 1.11.0 — Gmail (read-only): дайджест, поиск, /contact

Реализовано поэтапно (блоки A→F), см. `NEXT_STEPS.md` для полного
технического журнала по каждому блоку.

### Added
- **Gmail-интеграция, только чтение** — бот никогда не отправляет, не
  удаляет и не меняет почту. Отдельные креды и refresh token, не связаны
  с Google Calendar (`GMAIL_CLIENT_ID`/`GMAIL_CLIENT_SECRET`/
  `GMAIL_REFRESH_TOKEN`, опционально).
- **`scripts/gmail_auth.py`** — одноразовая настройка, loopback-flow с PKCE
  через OAuth-клиент типа Desktop app (не device-flow, как у Calendar:
  Google не разрешает Gmail-scope в «TVs and Limited Input devices»).
- **Сводка непрочитанных писем в утреннем дайджесте** — до 5 последних, с
  точным общим числом непрочитанных.
- **Локальный поисковый индекс почты** (`sqlite-vec`, отдельный файл
  `~/.tg_pa_bot/mail_index.db`) — инкрементальная переиндексация по
  расписанию (`MAIL_REINDEX_INTERVAL_MINUTES`, по умолчанию раз в час) и
  вручную (`/reindex_mail`). Retention 180 дней с автопрунингом
  (`MAIL_INDEX_RETENTION_DAYS`) — локально хранится не вся переписка,
  только последние полгода; сама почта при этом в Gmail не трогается.
- **`/search`** теперь ищет и по почте тоже (если Gmail настроен) —
  результаты из vault и почты объединяются и ранжируются вместе по
  смысловой близости, не как два отдельных списка.
- **`/contact`** — третий источник: письма от/про человека (точное
  совпадение подстроки по отправителю/теме/тексту письма — не семантика,
  как и у поиска по vault: для имён это надёжнее).

### Fixed
- Эмбеддинги (общий `EmbeddingClient`/дим-проба Ollama) раньше строились
  только при настроенном `OBSIDIAN_VAULT_PATH` — Gmail-only конфигурация
  (без Second Brain) осталась бы без эмбеддингов вообще, и почта никогда
  бы не индексировалась. Исправлено при разработке этой же фичи, до того
  как стало багом в проде.
- Аналогичный баг при wiring `search_service`/`contact_service` в
  `main.py` — доставались из `app.bot_data` внутри ветки, зависящей от
  vault, что привело бы к `NameError` при Gmail-only конфигурации.

### Technical
- Новые модули: `integrations/gmail_client.py` (`GoogleGmailClient` +
  `NullGmailClient`), `mail_index.py` (`MailIndex` — копия
  `vault_index.py`, но диффинг по id, а не по content-hash: письмо не
  меняется после получения), `services/mail_indexer.py` (`reindex_mail()`).
- 71 новый тест (было 226 к началу работы над Gmail, стало 297), mypy/ruff
  чистые на каждом шаге.

## 1.10.0 — Second Brain: семантический поиск по всему vault

Реализовано поэтапно (блоки A→F), см. `NEXT_STEPS.md` для полного
технического журнала по каждому блоку.

### Added
- **Семантический поиск по всему Obsidian vault**, не только по файлу
  задач. Эмбеддинги — через локальную **Ollama** (`/api/embed`), не
  sentence-transformers/fastembed: без тяжёлых Python-зависимостей, модель
  живёт в уже запущенной Ollama, а не в процессе бота.
- **`OBSIDIAN_VAULT_PATH`** — опциональная переменная окружения, включает
  Second Brain. Без неё бот ведёт себя ровно как раньше (grep по файлу
  задач) — фича полностью опциональна и деградирует мягко при недоступной
  Ollama.
- Векторное хранилище — **sqlite-vec** (`vec0`-таблица), без внешнего
  сервиса; файл индекса — `~/.tg_pa_bot/vault_index.db` (сознательно не
  внутри vault, чтобы не путать Obsidian Sync/git-бэкапы бинарником).
  Индекс — производный артефакт: пересобирается из markdown, потеря файла
  индекса не теряет данные.
- Инкрементальная переиндексация по расписанию (`VAULT_REINDEX_INTERVAL_MINUTES`,
  по умолчанию 60 минут) — эмбеддится только новое/изменённое (по хэшу
  содержимого чанка), удалённые заметки убираются из индекса.
- **`/reindex`** — ручной запуск переиндексации сейчас, не дожидаясь
  расписания. Отдельный скрипт `scripts/reindex_vault.py` для запуска без
  Telegram (например, сразу после настройки `OBSIDIAN_VAULT_PATH`).
- **`/contact <имя>`** — находит человека: упоминания по всему vault
  (обычный подстроковый поиск, не эмбеддинги — надёжнее для точных имён и
  работает даже без Ollama) + прошлые встречи с ним из Google Calendar за
  последние 90 дней (сопоставление по имени участника или email).
- `/search` теперь ищет по всему vault при настроенном
  `OBSIDIAN_VAULT_PATH` (раньше — только по файлу задач); без настройки
  поведение не изменилось.

### Changed
- `CalendarEvent` — новое поле `attendee_names` (отображаемое имя участника
  с фолбэком на email), параллельно существующему `attendees` (email) —
  без изменений в уже работающем коде.
- `scheduler/jobs.py`: `setup_scheduler()` больше не требует
  `TELEGRAM_CHAT_ID` целиком — job переиндексации регистрируется
  независимо от проактивных сообщений (иначе Second Brain не работал бы
  без настроенных проактивных уведомлений).

### Technical
- Новые модули: `vault_scanner.py` (сканирование и чанкинг markdown по
  заголовкам), `vault_index.py` (`VaultIndex` на sqlite-vec),
  `integrations/embedding_client.py` (`OllamaEmbeddingClient`),
  `services/vault_indexer.py` (`reindex_vault()` — diff/эмбеддинг/прунинг),
  `services/contact_service.py` (`ContactService`).
- Зависимость `sqlite-vec==0.1.9` (wheels под Linux/macOS/Windows x64, без
  torch).
- 66 новых тестов (было 160, стало 226), mypy/ruff чистые на каждом шаге.
